...
// --- FUNCIÓN 1: SOLO MÉDICOS (Ultra Rápida para la Agenda) ---
const getMedicosRaw = async () => {
  ...
  return rowsMedicos.map(row => ({
    ...
    email: row.get('Email') || "", // Asegúrate de crear esta columna en tu Google Sheet
    porcentajeComision: row.get('Comision') || "0" // Útil para el cálculo automático
  }));
};
...
// Mantenemos esta para compatibilidad, pero llama a las dos anteriores en paralelo
export const getCatalogos = async () => {
    const [medicos, servicios] = await Promise.all([
        getMedicos(),
        getServicios()
    ]);
    return { medicos, servicios };
};
