...
export const getOperacionCronograma = unstable_cache(
  ...
  ['op-cronograma-v1'],
  { revalidate: 300, tags: ['op-cronograma-v1'] } // 🚀 tags permite la revalidación bajo demanda
);
...
const getEquipoOperativoRaw = async () => {
  // Leemos la nueva pestaña sugerida
  const data = await getSheetData("EQUIPO_OPERATIVO");
  
  // Normalizamos para que el Formulario los lea igual que a los Médicos
  return data.map(item => ({
    nombre: item.Nombre || 'Sin Nombre',
    especialidad: item.Area || 'Administración',
    id: `OP-${item.Nombre}` // ID Prefijo para distinguir personal Operativo
  }));
};
