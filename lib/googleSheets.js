...
const getMedicosRaw = async () => {
  ...
  return rowsMedicos.map(row => ({
    ...
    email: row.get('Email') || "", 
    // Buscamos el dato en varias columnas posibles para asegurar que lo encuentre
porcentajeComision: row.get('Porcentaje Comisión') || row.get('Porcentaje Comision') || row.get('Comisión') || row.get('Comision') || "0"
  }));
};
