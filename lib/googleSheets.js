...
const getMedicosRaw = async () => {
  ...
  return rowsMedicos.map(row => {
    return {
      ...
      nombreCompleto: row.get('Nombre Completo'), // 🛠️ Crucial para el Map de Cambio de Turno
      nombre: row.get('Nombre Completo'),         // 🛡️ Retrocompatibilidad para otros módulos
      especialidad: row.get('Especialidad'),
      esquema: row.get('Esquema (Renta/Nómina)') || "Renta", // 🎯 Define el conteo SANSCE vs Externo
      color: row.get('Color Agenda'),
      reglasHorario: row.get('Reglas Horario') || "1,2,3,4,5|09:00-20:00",
      calendarId: row.get('Calendar ID'),
      email: row.get('Email') || "", 
      porcentajeComision: row.get('% Comisión') || "0" // 💰 Reparado: Ya no se mezcla con el esquema
    };
  });
  };
