...
export const getMedicos = unstable_cache(getMedicosRaw, ['medicos-data-v2'], { revalidate: 1 });
export const getServicios = unstable_cache(getServiciosRaw, ['servicios-data-v2'], { revalidate: 60 });

// --- FUNCIÓN PRINCIPAL (CORREGIDA): Obtiene Todo (Servicios + Médicos + Descuentos) ---
export async function getCatalogos() {
  try {
    // 1. Leemos Servicios y Médicos (Reutilizando lógica)
    const servicios = await getServicios(); 
    const medicos = await getMedicos();
    
    // 2. Leemos Descuentos (CORREGIDO)
    const rawDescuentos = await getSheetData("CATALOGO_DESCUENTOS");
    
    const descuentos = rawDescuentos
      .filter((d) => d.Activo === "TRUE" || d.Activo === true || d.Activo === "VERDADERO")
      .map((d) => {
        // Limpieza de valor: Quitar "%", "$" y "," antes de convertir a número
        let valorLimpio = d.Valor ? d.Valor.toString().replace(/[$,%]/g, '') : "0";
