...
async function getSheetData(sheetName) {
  try {
    ...
    // Convertimos las filas a objetos simples JSON con normalización de datos
    return rows.map(row => {
        const obj = {};
        row._worksheet.headerValues.forEach(header => {
            const val = row.get(header);
            // Si el encabezado dice "Fecha", aseguramos que sea un texto limpio
            obj[header] = (header.toLowerCase().includes('fecha') && val) 
...
const getOkrDataRaw = async (userEmail) => {
  try {
    ...
    // Paso C: Agrupar en Objetivos con Filtro de Estatus Activo
    const finalTree = objectives
        .filter(obj => {
            // 🛡️ PODER DE SELECCIÓN: Solo procesamos objetivos marcados como 'Activo' o 'SÍ'
            const estatus = String(obj.Estatus || obj.Activo || '').trim().toLowerCase();
            return estatus === 'activo' || estatus === 'si' || estatus === 'sí' || estatus === 'true';
        })
        .map(obj => {
            ...
            return {
                ...
                Color: obj.Color_Primario || '#4F46E5', // Color Sansce por defecto
                Promedio: objAverage,
                ResultadosClave: childKrs // Aquí viajan los KPIs correspondientes
            };
        });
